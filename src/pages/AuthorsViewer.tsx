...
export default function AuthorsViewer() {
    ...
    const fetchPoems = useCallback(async () => {
        ...
        authorsPoems = stored ? JSON.parse(stored) : [];
    ...
    const navToNewPoem = (direction: 'next' | 'prev') => {
        ...
        navigate(`/authors/viewer/${getLikeId(poems[new_index])}`);
        setIsLoading(false);
        setIsNew(true);
    };
            ...
            >
                <div className="flex flex-row items-center justify-between w-full">
                    <Button
                        ...
                        style={{ padding: 0 }}
                        onClick={() =>
                            navigate(`/authors/${encodeURIComponent(poem?.author ?? '')}`)
                        }
                    >
                        <MoveLeft />
                        Back to {poem?.author}
